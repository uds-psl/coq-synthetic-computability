...
Section PrenexNormalForm.
  ...
  (*
  Fixpoint conv_PNF_list `{falsity_flag} (f: form) : prod (list full_logic_quant) form :=
    match f with
    | falsity => ([], falsity)
    | atom P v => ([], atom P v)
    | bin op ϕ1 ϕ2 =>
      let (qs1, φ1) := conv_PNF_list ϕ1 in
      let (qs2, φ2) := conv_PNF_list ϕ2 in
      match op with
      | Impl => ((swap_quant qs1) ++ qs2,
        bin Impl
          φ1[shift (length qs2)]
          φ2[shift_quant (length qs2) (length qs1)]
          )
      | _ => (qs1 ++ qs2,
        bin op
          φ1[shift (length qs2)]
    ...
    | quant op ϕ => let (qs, φ) := conv_PNF_list ϕ in (op::qs, φ)
    end.
  *)
...
Lemma deMorganQuant_imp_DN :
  (forall X (p : X ->Prop), ~(forall x, p x) -> exists x, ~p x) -> DN.
Proof.
 clear. intros deM p nnP.
 now specialize (deM p (fun _ => False) nnP) as [H _].
Qed.
