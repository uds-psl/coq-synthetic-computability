...
From Undecidability Require Import simple principles.

Lemma hyper_simple_simple p :
  MP -> hyper_simple p -> simple p.
Proof.
  intros mp (Ep & Ip & nmp).
  repeat eapply conj; eauto.
  intros (q & H1 & H2 & H3).
  edestruct (dedekind_infinite_exceeds (compl p) q) as [f Hf].
  - eauto.
  - eapply MoreEnumerabilityFacts.generative_dedekind_infinite. 1-2:eauto.
    eapply semi_decidable_generative. 1: eauto. now eapply enum_iff.
    eauto.
  - eapply nmp. eexists. eapply exceeds_majorizes. eauto.
Qed.
